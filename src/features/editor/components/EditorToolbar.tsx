...
import ExportMenu from "./ExportMenu"; // <-- PREMIUM EXPORT ENGINE
import {
  Bold, Italic, Underline as UnderlineIcon, Strikethrough,
  AlignLeft, AlignCenter, AlignRight, AlignJustify,
  List, ListOrdered, Quote, Link as LinkIcon,
  Table, Image, Minus, ChevronDown, Type,
  Undo, Redo, Search, Download,
  Maximize2, Minimize2, ArrowLeft,
  ZoomIn, ZoomOut ,Code2
} from "lucide-react";
...
interface EditorToolbarProps {
  ...
  // New Props for Title & Back Button
  docTitle: string;
...
export default function EditorToolbar({
  ...
  // ── 1. Tiptap Re-render Engine & Debugging Logs ──
  ...
  // Active state logic for dropdown label
  ...
  useEffect(() => {
    if (!editor) {
      console.log("🔴 Toolbar Debug: Editor is NULL. Engine not connected yet.");
      return;
    }
    console.log("🟢 Toolbar Debug: Editor connected successfully!", editor);

    // Ye listener har keystroke ya format change par toolbar ko ZINDA karega
    const updateToolbar = () => {
      setRenderTick(t => t + 1);
    };
  ...
  // Agar editor load nahi hua tab bhi Back & Title bar show honi chahiye
  return (
    <div className="w-full bg-white/95 dark:bg-[#0B1120]/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 shadow-sm sticky top-0 z-40">
      {/* ── MAGIC FIX: gap-x-1 aur px-2 kiya taake normal screen par 1 line mein rahay ── */}
      <div className="w-full px-2 py-1.5 flex items-center gap-x-1 gap-y-2 flex-wrap relative">

        {/* ── BACK BUTTON & TITLE (New Space Saver) ── */}
        <div className="flex items-center gap-2 mr-2 shrink-0">
          ...
          <div className="min-w-[120px] max-w-[200px] flex items-center">
            {isEditingTitle ? (
              <input
                ref={titleInputRef}
                value={docTitle}
        ...
        <div className="relative flex items-center" ref={textDropdownRef}>
          <button
            ...
            title="Text Style"
          >
            <span className="w-[85px] text-left truncate">{getCurrentTextStyle()}</span>
            <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isTextDropdownOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          {isTextDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl overflow-hidden z-50 py-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
              {[
                ...
                { label: "Title (H1)", action: () => editor?.chain().focus().toggleHeading({ level: 1 }).run(), active: editor?.isActive('heading', { level: 1 }) },
                ...
                { label: "Heading 5", action: () => editor?.chain().focus().toggleHeading({ level: 5 }).run(), active: editor?.isActive('heading', { level: 5 }) },
                { label: "Heading 6", action: () => editor?.chain().focus().toggleHeading({ level: 6 }).run(), active: editor?.isActive('heading', { level: 6 }) },
              ].map((item, idx) => (
                <button
                  key={idx}
                  onMouseDown={(e) => {
                    e.preventDefault(); // Stop focus steal
                    item.action();      // Run command IMMEDIATELY
                    setIsTextDropdownOpen(false); // Then close dropdown
                  }}
        ...
        <div className="relative flex items-center shrink-0" ref={fontSizeDropdownRef}>
          <button
            onMouseDown={(e) => e.preventDefault()}
            ...
            title="Font Size"
          >
            {/* Fallback to '16' if attribute is missing */}
            <span className="w-[3ch] text-center">
              {editor?.getAttributes('textStyle')?.fontSize?.replace('px', '') || '16'}
            </span>
            <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform ${isFontSizeDropdownOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          {isFontSizeDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl overflow-hidden z-50 py-1.5 animate-in fade-in slide-in-from-top-1 duration-150 max-h-48 overflow-y-auto custom-content-scroll">
              {["12", "14", "16", "18", "20", "24", "28", "32", "36"].map((size, idx) => {
                const isActive = editor?.getAttributes('textStyle')?.fontSize === `${size}px` || (!editor?.getAttributes('textStyle')?.fontSize && size === "16");
                return (
                  <button
                    key={idx}
                    onMouseDown={(e) => {
          ...
          <div className="relative flex items-center" ref={alignDropdownRef}>
            ...
            {isAlignDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl overflow-hidden z-50 py-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
                {[
                  { label: "Left", icon: <AlignLeft className="w-4 h-4" />, action: () => editor?.chain().focus().setTextAlign('left').run(), active: editor?.isActive({ textAlign: 'left' }) },
                  { label: "Center", icon: <AlignCenter className="w-4 h-4" />, action: () => editor?.chain().focus().setTextAlign('center').run(), active: editor?.isActive({ textAlign: 'center' }) },
                  { label: "Right", icon: <AlignRight className="w-4 h-4" />, action: () => editor?.chain().focus().setTextAlign('right').run(), active: editor?.isActive({ textAlign: 'right' }) },
                ].map((item, idx) => (
                  <button
                    key={idx}
                    onMouseDown={(e) => {
        ...
        <ToolbarGroup>
          <div className="relative flex items-center gap-1 px-1 group cursor-pointer" title="Text Color">
            <span className="text-xs font-black text-slate-500">A</span>
            <input type="color" className="w-4 h-4 rounded cursor-pointer border-0 bg-transparent p-0" onChange={(e) => editor?.chain().focus().setColor(e.target.value).run()} />
          </div>
          <div className="relative flex items-center gap-1 px-1 group cursor-pointer" title="Highlight Color">
            <span className="text-xs font-black text-slate-500 bg-yellow-200 dark:bg-yellow-500/20 px-0.5 rounded">H</span>
            <input type="color" defaultValue="#fef08a" className="w-4 h-4 rounded cursor-pointer border-0 bg-transparent p-0" onChange={(e) => editor?.chain().focus().toggleHighlight({ color: e.target.value }).run()} />
          </div>
        ...
        {/* MAGIC FIX: editor ke baad ? laga diya taake null par crash na ho */}
        <ToolBtn 
        ...
        {/* Table & Media */}
        <ToolbarGroup>
          {/* ── LEVEL 100 SMART TABLE CONTROLS ── */}
          <TableControls editor={editor} disabled={isReadOnly} />
          
          <ToolBtn onClick={onImageUpload} disabled={isReadOnly} title="Insert Image">
            <Image className="w-4 h-4" />
          </ToolBtn>
          <ToolBtn onClick={() => editor?.chain().focus().setHorizontalRule().run()} disabled={isReadOnly} title="Divider Line">
        ...
        <Divider />
        <ToolbarGroup>
          <ToolBtn onClick={() => setZoomLevel(Math.max(50, zoomLevel - 10))} disabled={zoomLevel <= 50} title="Zoom Out">
          ...
          <ToolBtn onClick={() => setZoomLevel(Math.min(200, zoomLevel + 10))} disabled={zoomLevel >= 200} title="Zoom In">
        ...
        <ToolbarGroup>
          ...
          {/* ── LEVEL 100 EXPORT MENU ── */}
          <ExportMenu editor={editor} docTitle={docTitle} disabled={false} />

          <ToolBtn onClick={onToggleFocusMode} active={isFocusMode} title="Focus Mode">
            {isFocusMode ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
          </ToolBtn>
        </ToolbarGroup>
      </div>

    {/* ── Premium Link Modal (Portal to bypass z-index issues) ── */}
      {showLinkModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-80 p-5 animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Insert Link</h3>
            <input 
              autoFocus
              type="url" 
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowLinkModal(false)} className="px-4 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors">Cancel</button>
              <button onClick={applyLink} className="px-4 py-1.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm transition-colors">Apply</button>
            </div>
          </div>
        </div>,
        document.body
...
function Divider() {
  return <div className="w-px h-5 bg-slate-200 dark:bg-slate-700 mx-0.5 sm:mx-1 shrink-0" />;
}
function ToolBtn({ children, onClick, active = false, disabled = false, title }: { children: React.ReactNode; onClick: () => void; active?: boolean; disabled?: boolean; title?: string; }) {
  return (
    <button 
      onClick={(e) => {
        console.log(`🔘 Action Triggered: ${title}`); // <-- Debug log
        onClick();
      }}
      onMouseDown={(e) => e.preventDefault()} // <-- MAGIC FIX: Button ko focus steal karne se rokega!
      disabled={disabled} 
      title={title} 
