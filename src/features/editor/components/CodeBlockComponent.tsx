...
export default function CodeBlockComponent({ node, updateAttributes, extension }: any) {
  ...
  const copyToClipboard = () => {
    // node.textContent pure code ko bina HTML tags ke copy karta hai
  ...
  return (
    <NodeViewWrapper className="relative group rounded-xl overflow-hidden bg-slate-900 dark:bg-[#0d1117] text-slate-50 my-6 border border-slate-800 shadow-md">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-slate-800/50 dark:bg-white/5 border-b border-slate-700/50">
        ...
        {/* Language Selector */}
        <select
          contentEditable={false} // TipTap isko edit hone se rokega
        ...
        >
          <option value="null" className="bg-slate-800 text-white">Plain Text</option>
          <option value="javascript" className="bg-slate-800 text-white">JavaScript</option>
          <option value="typescript" className="bg-slate-800 text-white">TypeScript</option>
          <option value="html" className="bg-slate-800 text-white">HTML</option>
          <option value="css" className="bg-slate-800 text-white">CSS</option>
          <option value="python" className="bg-slate-800 text-white">Python</option>
          <option value="rust" className="bg-slate-800 text-white">Rust</option>
          <option value="bash" className="bg-slate-800 text-white">Terminal</option>
        </select>
